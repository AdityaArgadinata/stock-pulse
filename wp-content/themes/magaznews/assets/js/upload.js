...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Magaznews_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    magaznews_file_frame = wp.media.frames.magaznews_file_frame = wp.media({
      button: {
    ...
    magaznews_file_frame.on( 'select', function() {

      var state = magaznews_file_frame.state('author-info-insert-image');
