<?php
if ( ! class_exists( 'Magaznews_List_Posts_Widget' ) ) {
	/**
	 * Adds Magaznews List Widget.
	 */
	class Magaznews_List_Posts_Widget extends WP_Widget {
		...
		public function __construct() {
			$list_widget = array(
				'classname'   => 'magaznews-widget list-posts-widget',
				'description' => __( 'Retrive List Widgets', 'magaznews' ),
			);
			parent::__construct(
				'magaznews_list_posts_widget',
				__( 'Artify Widget: List Widget', 'magaznews' ),
				$list_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$list_btn_label   = ( ! empty( $instance['button_label'] ) ) ? $instance['button_label'] : '';
			...
			$list_btn_link    = ( ! empty( $instance['button_link'] ) ) ? $instance['button_link'] : esc_url( get_category_link( $list_category ) );

			echo $args['before_widget'];

			if ( ! empty( $list_title || $list_btn_label ) ) {
				?>
				<div class="header-title">
					<?php echo $args['before_title'] . esc_html( $list_title ) . $args['after_title']; ?>
					<?php if ( ! empty( $list_btn_label ) ) : ?>
						<a href="<?php echo esc_url( $list_btn_link ); ?>" class="view-all"><?php echo esc_html( $list_btn_label ); ?></a>
					<?php endif; ?>
				</div>
			<?php } ?>
		...
		public function form( $instance ) {
			...
			$list_btn_label   = isset( $instance['button_label'] ) ? $instance['button_label'] : '';
			$list_btn_link    = isset( $instance['button_link'] ) ? $instance['button_link'] : '';
			$list_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$list_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			?>
			<p>
			...
			<p>
				...
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_label' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_label' ) ); ?>" type="text" value="<?php echo esc_attr( $list_btn_label ); ?>" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>"><?php esc_html_e( 'View All Button URL:', 'magaznews' ); ?></label>
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_link' ) ); ?>" type="text" value="<?php echo esc_attr( $list_btn_link ); ?>" />
			</p>
			<p>
