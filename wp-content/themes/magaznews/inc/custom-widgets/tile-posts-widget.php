<?php
if ( ! class_exists( 'Magaznews_Tile_Posts_Widget' ) ) {
	/**
	 * Adds Magaznews Tile Widget.
	 */
	class Magaznews_Tile_Posts_Widget extends WP_Widget {
		...
		public function __construct() {
			$tile_posts_widget = array(
				'classname'   => 'magaznews-widget tile-posts-widget',
				'description' => __( 'Retrive Tile Widgets', 'magaznews' ),
			);
			parent::__construct(
				'magaznews_tile_posts_widget',
				__( 'Artify Widget: Tile Widget', 'magaznews' ),
				$tile_posts_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$tile_posts_btn_label   = ( ! empty( $instance['button_label'] ) ) ? $instance['button_label'] : '';
			...
			$tile_posts_btn_link    = ( ! empty( $instance['button_link'] ) ) ? $instance['button_link'] : esc_url( get_category_link( $tile_posts_category ) );

			echo $args['before_widget'];

			if ( ! empty( $tile_posts_title || $tile_posts_btn_label ) ) {
				?>
				<div class="header-title">
					<?php echo $args['before_title'] . esc_html( $tile_posts_title ) . $args['after_title']; ?>
					<?php if ( ! empty( $tile_posts_btn_label ) ) : ?>
						<a href="<?php echo esc_url( $tile_posts_btn_link ); ?>" class="view-all"><?php echo esc_html( $tile_posts_btn_label ); ?></a>
					<?php endif; ?>
				</div>
			<?php } ?>
		...
		public function form( $instance ) {
			...
			$tile_posts_btn_label   = isset( $instance['button_label'] ) ? $instance['button_label'] : '';
			$tile_posts_btn_link    = isset( $instance['button_link'] ) ? $instance['button_link'] : '';
			$tile_posts_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$tile_posts_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			?>
			<p>
			...
			<p>
				...
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_label' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_label' ) ); ?>" type="text" value="<?php echo esc_attr( $tile_posts_btn_label ); ?>" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>"><?php esc_html_e( 'View All Button URL:', 'magaznews' ); ?></label>
				<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'button_link' ) ); ?>" type="text" value="<?php echo esc_attr( $tile_posts_btn_link ); ?>" />
			</p>
			<p>
